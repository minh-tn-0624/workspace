...
#include "scan_matching_pkg/visualization.h"
...
class ScanProcessor {
  ...
  public:
    ...
    void handleLaserScan(const sensor_msgs::LaserScan::ConstPtr& msg) {
      ...
      curr_trans = Transform(); //use guess transform to transform coordinates of curr scan
      ...
      while (count < MAX_ITER && (curr_trans != prev_trans || count==0)) {
        ...
        prev_trans = curr_trans; //pass curr_trans to prev_trans
        ++count;

        // **************************************** We update the transforms here ******************************************* ////
        updateTransform(corresponds, curr_trans);//optimize to find a new transform
      ...
      ROS_INFO("Transform tx = %f, ty = %f, theta = %f",curr_trans.x_disp, curr_trans.y_disp, curr_trans.theta_rot);

      this->global_tf = global_tf * curr_trans.getMatrix();

      publishPos();
      prev_points = points; //points is the previous scan set for next time
    }

    // Handles reading of scan, pushes fills points vector
    //purpose: Read lidar dato to Point structure
